...
static void Preferences_SetMenuItems( void ) {
	s_preferences.crosshair.curvalue		= ( int )uiLocal->Cvar_VariableValue( "cg_drawCrosshair" ) % NUM_CROSSHAIRS;
	s_preferences.simpleitems.curvalue		= uiLocal->Cvar_VariableValue( "cg_simpleItems" ) != 0;
	s_preferences.brass.curvalue			= uiLocal->Cvar_VariableValue( "cg_brassTime" ) != 0;
	s_preferences.wallmarks.curvalue		= uiLocal->Cvar_VariableValue( "cg_marks" ) != 0;
	s_preferences.identifytarget.curvalue	= uiLocal->Cvar_VariableValue( "cg_drawCrosshairNames" ) != 0;
	s_preferences.dynamiclights.curvalue	= uiLocal->Cvar_VariableValue( "r_dynamiclight" ) != 0;
	s_preferences.highqualitysky.curvalue	= uiLocal->Cvar_VariableValue ( "r_fastsky" ) == 0;
	s_preferences.synceveryframe.curvalue	= uiLocal->Cvar_VariableValue( "r_finish" ) != 0;
	s_preferences.forcemodel.curvalue		= uiLocal->Cvar_VariableValue( "cg_forcemodel" ) != 0;
	s_preferences.drawteamoverlay.curvalue	= Com_Clamp( 0, 3, uiLocal->Cvar_VariableValue( "cg_drawTeamOverlay" ) );
	s_preferences.allowdownload.curvalue	= uiLocal->Cvar_VariableValue( "cl_allowDownload" ) != 0;
}
...
static void Preferences_Event( void * ptr, int notification ) {
	...
	switch ( ( ( menucommon_s * )ptr )->id ) {
		case ID_CROSSHAIR:
			...
			uiLocal->Cvar_SetValue( "cg_drawCrosshair", s_preferences.crosshair.curvalue );
			break;

		case ID_SIMPLEITEMS:
			uiLocal->Cvar_SetValue( "cg_simpleItems", s_preferences.simpleitems.curvalue );
			break;

		case ID_HIGHQUALITYSKY:
			uiLocal->Cvar_SetValue( "r_fastsky", !s_preferences.highqualitysky.curvalue );
			break;

		case ID_EJECTINGBRASS:
			if ( s_preferences.brass.curvalue ) {
				uiLocal->Cvar_Reset( "cg_brassTime" );
			} else {
				uiLocal->Cvar_SetValue( "cg_brassTime", 0 );
			}
			break;

		case ID_WALLMARKS:
			uiLocal->Cvar_SetValue( "cg_marks", s_preferences.wallmarks.curvalue );
			break;

		case ID_DYNAMICLIGHTS:
			uiLocal->Cvar_SetValue( "r_dynamiclight", s_preferences.dynamiclights.curvalue );
			break;

		case ID_IDENTIFYTARGET:
			uiLocal->Cvar_SetValue( "cg_drawCrosshairNames", s_preferences.identifytarget.curvalue );
			break;

		case ID_SYNCEVERYFRAME:
			uiLocal->Cvar_SetValue( "r_finish", s_preferences.synceveryframe.curvalue );
			break;

		case ID_FORCEMODEL:
			uiLocal->Cvar_SetValue( "cg_forcemodel", s_preferences.forcemodel.curvalue );
			break;

		case ID_DRAWTEAMOVERLAY:
			uiLocal->Cvar_SetValue( "cg_drawTeamOverlay", s_preferences.drawteamoverlay.curvalue );
			break;

		case ID_ALLOWDOWNLOAD:
			uiLocal->Cvar_SetValue( "cl_allowDownload", s_preferences.allowdownload.curvalue );
			uiLocal->Cvar_SetValue( "sv_allowDownload", s_preferences.allowdownload.curvalue );
			break;
...
void Preferences_Cache( void ) {
	...
	for ( n = 0; n < NUM_CROSSHAIRS; n++ ) {
		s_preferences.crosshairShader[n] = uiLocal->re_RegisterShaderNoMip( va( "gfx/2d/crosshair%c", 'a' + n ) );
	}
}
