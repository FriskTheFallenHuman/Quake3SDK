...
static int CG_TeamScoreboard( int y, team_t team, float fade, int maxClients, int lineHeight ) {
	...
	for ( i = 0 ; i < cg.numScores && count < maxClients ; i++ ) {
		...
		qboolean useLargeFormat = lineHeight == SB_NORMAL_HEIGHT ? qtrue : qfalse;

		CG_DrawClientScore( y + lineHeight * count, score, color, fade, useLargeFormat );
	...
	if ( !localClient ) {
		...
		for ( i = 0 ; i < cg.numScores ; i++ ) {
			if ( cg.scores[i].client == cg.snap->ps.clientNum ) {
				qboolean useLargeFormat = lineHeight == SB_NORMAL_HEIGHT ? qtrue : qfalse;
				CG_DrawClientScore( y, &cg.scores[i], fadeColor, fade, useLargeFormat );
				break;
			}
		}
	}
