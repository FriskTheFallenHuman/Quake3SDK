...
void G_MissileImpact( gentity_t * ent, trace_t * trace ) {
	...
	if ( !strcmp( ent->classname, "hook" ) ) {
		...
		gameLocal->LinkEntity( (sharedEntity_t *)ent );
		gameLocal->LinkEntity( (sharedEntity_t *)nent );
...
void G_RunMissile( gentity_t * ent ) {
	...
	gameLocal->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, ent->clipmask, false );

	if ( tr.startsolid || tr.allsolid ) {
		// make sure the tr.entityNum is set to the entity we're stuck in
		gameLocal->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, passent, ent->clipmask, false );
		tr.fraction = 0;
	} else {
	...
	if ( ent->s.weapon == WP_PROX_LAUNCHER && !ent->count ) {
		...
		gameLocal->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, ENTITYNUM_NONE, ent->clipmask );
