...
static void PlayerSettings_SaveChanges( void ) {
	// name
	uiLocal->Cvar_Set( "name", s_playersettings.name.field.buffer );

	// handicap
	uiLocal->Cvar_SetValue( "handicap", 100 - s_playersettings.handicap.curvalue * 5 );

	// effects color
	uiLocal->Cvar_SetValue( "color1", uitogamecode[s_playersettings.effects.curvalue] );
}
...
static void PlayerSettings_SetMenuItems( void ) {
	...
	// effects color
	c = uiLocal->Cvar_VariableValue( "color1" ) - 1;
	if ( c < 0 || c > 6 ) {
		c = 6;
	}
	...
	// handicap
	h = Com_Clamp( 5, 100, uiLocal->Cvar_VariableValue( "handicap" ) );
...
static void PlayerSettings_MenuEvent( void * ptr, int event ) {
	...
	switch ( ( ( menucommon_s * )ptr )->id ) {
		case ID_HANDICAP:
			uiLocal->Cvar_Set( "handicap", va( "%i", 100 - 25 * s_playersettings.handicap.curvalue ) );
			break;
...
void PlayerSettings_Cache( void ) {
	...
	uiLocal->re_RegisterShaderNoMip( ART_MODEL0 );
	uiLocal->re_RegisterShaderNoMip( ART_MODEL1 );
	uiLocal->re_RegisterShaderNoMip( ART_BACK0 );
	uiLocal->re_RegisterShaderNoMip( ART_BACK1 );

	s_playersettings.fxBasePic = uiLocal->re_RegisterShaderNoMip( ART_FX_BASE );
	s_playersettings.fxPic[0] = uiLocal->re_RegisterShaderNoMip( ART_FX_RED );
	s_playersettings.fxPic[1] = uiLocal->re_RegisterShaderNoMip( ART_FX_YELLOW );
	s_playersettings.fxPic[2] = uiLocal->re_RegisterShaderNoMip( ART_FX_GREEN );
	s_playersettings.fxPic[3] = uiLocal->re_RegisterShaderNoMip( ART_FX_TEAL );
	s_playersettings.fxPic[4] = uiLocal->re_RegisterShaderNoMip( ART_FX_BLUE );
	s_playersettings.fxPic[5] = uiLocal->re_RegisterShaderNoMip( ART_FX_CYAN );
	s_playersettings.fxPic[6] = uiLocal->re_RegisterShaderNoMip( ART_FX_WHITE );
}
