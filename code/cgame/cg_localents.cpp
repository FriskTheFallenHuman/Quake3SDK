...
void CG_FragmentBounceSound( localEntity_t * le, trace_t * trace ) {
	if ( le->leBounceSoundType == LEBS_BLOOD ) {
		...
		if ( rand() & 1 ) {
			...
			cgameLocal->S_StartSound( trace->endpos, ENTITYNUM_WORLD, CHAN_AUTO, s );
		}
...
void CG_AddFragment( localEntity_t * le ) {
	...
	if ( le->pos.trType == TR_STATIONARY ) {
		...
		if ( t < SINK_TIME ) {
			...
			cgameLocal->R_AddRefEntityToScene( &le->refEntity );
	...
	// and floating levels
	if ( cgameLocal->CM_PointContents( trace.endpos, 0 ) & CONTENTS_NODROP ) {
...
void CG_AddFadeRGB( localEntity_t * le ) {
	...
	cgameLocal->R_AddRefEntityToScene( re );
}
...
static void CG_AddExplosion( localEntity_t * ex ) {
	...
	if ( ex->light ) {
		...
		cgameLocal->R_AddLightToScene( ent->origin, light, ex->lightColor[0], ex->lightColor[1], ex->lightColor[2] );
	}
}
...
static void CG_AddSpriteExplosion( localEntity_t * le ) {
	...
	if ( le->light ) {
		...
		cgameLocal->R_AddLightToScene( re.origin, light, le->lightColor[0], le->lightColor[1], le->lightColor[2] );
	}
}
...
//			cgameLocal->S_StartSound (re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeExplodeSound );
			cgameLocal->S_StartLocalSound( cgs.media.kamikazeExplodeSound, CHAN_AUTO );
		...
		cgameLocal->R_AddRefEntityToScene( &shockwave );
	}
...
//				cgameLocal->S_StartSound (re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeImplodeSound );
				cgameLocal->S_StartLocalSound( cgs.media.kamikazeImplodeSound, CHAN_AUTO );
		...
		cgameLocal->R_AddRefEntityToScene( re );
		// add the dlight
		cgameLocal->R_AddLightToScene( re->origin, c * 1000.0, 1.0, 1.0, c );
	}
