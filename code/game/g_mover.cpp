...
void Think_SetupTrainTargets( gentity_t * ent ) {
	...
	if ( !ent->nextTrain ) {
		G_Warning ( "func_train at %s with an unfound target\n",
					vtos( ent->r.absmin ) );
		return;
	}
	...
	for ( path = ent->nextTrain ; path != start ; path = next ) {
		...
		if ( !path->target ) {
			G_Warning ( "Train corner at %s without a target\n",
						vtos( path->s.origin ) );
			return;
		}
		...
		do {
			...
			if ( !next ) {
				G_Warning ( "Train corner at %s without a target path_corner\n",
							vtos( path->s.origin ) );
				return;
			}
...
void SP_path_corner( gentity_t * self ) {
	if ( !self->targetname ) {
		G_Warning ( "path_corner with no targetname at %s\n", vtos( self->s.origin ) );
		G_FreeEntity( self );
		return;
	}
...
void SP_func_train ( gentity_t * self ) {
	...
	if ( !self->target ) {
		G_Warning ( "func_train without a target at %s\n", vtos( self->r.absmin ) );
		G_FreeEntity( self );
		return;
	}
