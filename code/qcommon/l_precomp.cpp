																	...
																	double *floatvalue, int integer)
{
	...
	while(!error && firstoperator)
	{
		...
		for (o = firstoperator; o->next; o = o->next)
		{
			...
			if (o->boperator != P_LOGIC_NOT
					&& o->boperator != P_BIN_NOT) v = v->next;
		...
		if (integer)
		{
			Log_Write("operator %s, value1 = %d", PunctuationFromNum(source->scriptstack, o->boperator), v1->intvalue);
...
#endif //DEBUG_EVAL
		if (error) break;
		lastoperatortype = o->boperator;
		//if not an operator with arity 1
		if (o->boperator != P_LOGIC_NOT
				&& o->boperator != P_BIN_NOT)
		{
			//remove the second value if not question mark operator
			if (o->boperator != P_QUESTIONMARK) v = v->next;
			//
