...
void trap_LocateGameData( gentity_t * gEnts, int numGEntities, int sizeofGEntity_t, playerState_t * clients, int sizeofGClient ) {
	gameSyscall->LocateGameData( ( sharedEntity_t * )gEnts, numGEntities, sizeofGEntity_t, clients, sizeofGClient );
}
...
void trap_SetBrushModel( gentity_t * ent, const char * name ) {
	gameSyscall->SetBrushModel( ( sharedEntity_t * )ent, name );
}
...
void trap_LinkEntity( gentity_t * ent ) {
	gameSyscall->LinkEntity( ( sharedEntity_t * )ent );
}

void trap_UnlinkEntity( gentity_t * ent ) {
	gameSyscall->UnlinkEntity( ( sharedEntity_t * )ent );
}

int trap_EntitiesInBox( vec3_t mins, vec3_t maxs, int * list, int maxcount ) {
